	...
	$(".fullscreen_content")
			.append(
					"" + "<iframe width='420' height='315'"
							...
							+ $("#videoimg_" + id).text()
							+ ".png' class='video_icon  tooltip_guide profile'></img>"

							+ "	<div class='like_count' style='margin-top: 3px;'> <p  style='    margin: 0;' class='like_"
							+ id
							+ "' >"
							+ $("#videolikes_" + id).text()
							+ "</p></div>"
							+ "	<button  style='    float: right;' id='videoid_"
							+ id
							...
							+ $("#videoheader_" + id).text()
							+ " </h2>"
							+ "<p > "
							+ $("#videocontent_" + id).text()
							+ "</p></div>" + "</div>");
...
function setLike(message, id, type, commentId) {
	console.log(message);
	if (type) {
		$("up_" + id).disabled = true;
	} else {
		$("down_" + id).disabled = true;
	}
...
if ($('#auth').text() == "true") {
	...
	// var logintrigger = document.getElementById("video_b"), logindialog =
	// document
...
for (kk = 0; kk < edata.length; kk++) {
	if (edata[kk].state == true) {
		$(".up" + edata[kk].id).disabled = true;
	} else {
		$(".down" + edata[kk].id).disabled = true;
	}
...
$('.discussion_type').on(
		'change',
		function() {
			window.location.replace("/discussions/" + $('#main_name').text()
					+ "/1/" + this.value);
			$('#content').text("");
		...
		$("#content")
				.append(
						...
						+ "<li style='text-align: left;'>" + "					<div>"
								...
								+ message[i].user.id
								+ "'"
								+ "						th:src='http://ddragon.leagueoflegends.com/cdn/6.22.1/img/profileicon/"
								+ message[i].user.summoner.image
								+ ".png'"
